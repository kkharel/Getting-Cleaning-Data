...
# Reading from HDF5 
  # for storing large datasets. heirarchial data format
...
df = data.frame(1L:5L, sql(0,1, length.out = 5),
                c("ab", "cde", "fghi", "a", "s"),
...
# Getting data off of webpages with readLines()
...
# When authentication is required then,
...
json2 = jsonlite::fromJSON(toJSON(json1)) # creating dataframe from json object
json2[1,1:4] # first row and first 4 columns

# Read twitter Documentation

# httr allows GET, POST, PUT, DELETE requests if we are authorized. httr works well with FB, Google, Github etc..


# Foreign package loads data from Minitab, SAS, SPSS, Stata etc
